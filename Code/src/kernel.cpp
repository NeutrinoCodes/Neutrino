...
)
{
  ...
  compiler_options                      = "";
  loc_options                           = new char[compiler_options.size () + 1]();                 // Building temporary char options buffer...
  loc_kernel_source                     = new char*[loc_kernel_file_name.size ()]();                // Building temporary source char buffer...
  loc_kernel_source_size                = new size_t ();                                            // Building temporary char options buffer...

  compiler_options.copy (loc_options, compiler_options.size () + 1);                                // Building options string...
  ...
  for(i = 0; i < loc_kernel_file_name.size (); i++)
  {
    ...
    kernel_source[i].copy (loc_kernel_source[i], kernel_source[i].size () + 1);                     // Building string source buffer...
    loc_kernel_source[i][loc_kernel_source_size[i]] = '\0';                                         // Null terminating buffer string...
  }
