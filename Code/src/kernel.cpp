...
void kernel::build ()
{
  ...
  neutrino::action ("enumerating OpenCL kernel ID...");                                             // Printing message...

  for(i = 0; i < neutrino::k_num; i++)                                                              // Scanning OpenCL kernel argument array...
  {
    if(neutrino::kernel_id[i] == NULL)                                                              // Looking for 1st non-assigned OpenCL kernel ID...
    {
      neutrino::kernel_id[i] = kernel_id;                                                           // Assigning value to 1st non-assigned OpenCL kernel ID...
      break;                                                                                        // Exiting loop...
    }
  }
