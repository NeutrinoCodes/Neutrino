                  ...
                  )
{
  ...
  for (i = 0; i < num_char; i++)                                                // Looping on all characters in input string...
  {
    ...
    {
      data_index[k + j] = loc_neutrino->neutrino_font->font_index[char_item[i]] + j;                // Computing index of font data for character in input string...
    }
  ...
  for (i = 0; i < num_char; i++)                                                // Looping on all characters in input string...
  {
    ...
    shift += loc_neutrino->neutrino_font->font_width[char_item[i]];                                 // Accumulating character shift...
  }
  ...
  for (i = 0; i < num_points; i++)                                              // Unwrapping text data...
  {
    ...
    x[i] = ASCII_SCALE*                                                         // Applying scaling...
           (loc_neutrino->neutrino_font->font_vertex[data_index[even]] -                            // Setting vertex data...
            loc_neutrino->neutrino_font->font_vertex[data_index[0]] +                               // Subtracting base point...
            offset[i] +                                                         // Adding offset...
            kern[i]);                                                           // Adding kern...

    y[i] = loc_neutrino->neutrino_font->font_vertex[data_index[odd]]*ASCII_SCALE;                   // Setting "y" data...
