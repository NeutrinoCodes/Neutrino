                  ...
                  )
{
  ...
  switch(loc_device_type)                                                           // Selecting device type...
  {
    case NU_CPU:
      ...
      strcpy (device_type_text, "NU_CPU");                                             // Setting device_type_text...
      break;

    case NU_GPU:
      device_type      = CL_DEVICE_TYPE_GPU;                                        // Setting device type = NU_GPU...
      strcpy (device_type_text, "NU_GPU");                                             // Setting device_type_text...
      break;

    case NU_ACCELERATOR:
      device_type      = CL_DEVICE_TYPE_ACCELERATOR;                                // Setting device type = NU_ACCELERATOR...
      strcpy (device_type_text, "NU_ACCELERATOR");                                     // Setting device_type_text...
      break;

    case NU_DEFAULT:
      device_type      = CL_DEVICE_TYPE_DEFAULT;                                    // Setting device type = NU_DEFAULT...
      strcpy (device_type_text, "NU_DEFAULT");                                         // Setting device_type_text...
      break;

    case NU_ALL:
      device_type      = CL_DEVICE_TYPE_ALL;                                        // Setting device type = NU_ALL...
      strcpy (device_type_text, "NU_ALL");                                             // Setting device_tyep_text...
      break;

    default:
      device_type      = CL_DEVICE_TYPE_DEFAULT;                                    // Setting device type = NU_DEFAULT...
      device_type_text = (char*) "NU_DEFAULT";                                         // Printing message...
      break;
  }
  ...
  for(i = 0; i < platforms_number; i++)                                             // Checking all platforms:
  {
    ...
    printf (
            "        --> profile:    %s\n",
            opencl_platform[i]->profile->value
           );                                                                       // Printing message...
    printf (
            "        --> version:    %s\n",
            opencl_platform[i]->version->value
           );                                                                       // Printing message...
    printf (
            "        --> name:       %s\n",
            opencl_platform[i]->name->value
           );                                                                       // Printing message...
    printf (
            "        --> vendor:     %s\n",
            opencl_platform[i]->vendor->value
           );                                                                       // Printing message...
  ...
  for(i = 0; i < devices_number; i++)                                               // Checking all devices:
  {
    ...
    printf (
            "        --> device name:     %s\n",
            opencl_device[i]->name->value
           );                                                                       // Printing message...
    printf (
            "        --> device platform: %s\n",
            opencl_device[i]->profile->value
           );                                                                       // Printing message...
  }
